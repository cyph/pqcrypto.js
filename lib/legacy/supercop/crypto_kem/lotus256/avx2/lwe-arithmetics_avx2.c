...
void submat_negmul(U16 *C, const U16 *A, const U16 *B){
  ...
  for(i = 0; i < hn; ++i){
    ...
    c[6] = _mm256_setzero_si256();
    c[7] = _mm256_setzero_si256();
    p = B;
    for(j = 0; j < hn; ++j){
      ...
      b[0] = _mm256_loadu_si256((__m256i*)(p + 64));
      b[1] = _mm256_loadu_si256((__m256i*)(p + 80));
      b[2] = _mm256_loadu_si256((__m256i*)(p + 96));
      b[3] = _mm256_loadu_si256((__m256i*)(p + 112));
      b[0] = _mm256_mullo_epi16(b[0], a);
      b[1] = _mm256_mullo_epi16(b[1], a);
      b[2] = _mm256_mullo_epi16(b[2], a);
      b[3] = _mm256_mullo_epi16(b[3], a);
      c[4] = _mm256_sub_epi16(c[4], b[0]);
      c[5] = _mm256_sub_epi16(c[5], b[1]);
      c[6] = _mm256_sub_epi16(c[6], b[2]);
      c[7] = _mm256_sub_epi16(c[7], b[3]);
      p += hl;
    }
    ...
    _mm256_storeu_si256((__m256i*)(C + 96), c[6]);
    _mm256_storeu_si256((__m256i*)(C + 112), c[7]);
    A += hn; C += hl;
  }
}
...
void submat_submul(U16 *C, const U16 *A, const U16 *B){
  ...
  for(i = 0; i < hn; ++i){
    ...
    c[6] = _mm256_loadu_si256((__m256i*)(C + 96));
    c[7] = _mm256_loadu_si256((__m256i*)(C + 112));
    p = B;
    for(j = 0; j < hn; ++j){
      a = _mm256_set1_epi16(A[j]);
      b[0] = _mm256_loadu_si256((__m256i*)(p));
...
void submat_negsubmul(U16 *C, const U16 *A, const U16 *B){
  ...
  for(i = 0; i < hn; ++i){
    ...
    c[6] = _mm256_sub_epi16(m, c[6]);
    c[7] = _mm256_sub_epi16(m, c[7]);
    p = B;
    for(j = 0; j < hn; ++j){
      a = _mm256_set1_epi16(A[j]);
      b[0] = _mm256_loadu_si256((__m256i*)(p));
