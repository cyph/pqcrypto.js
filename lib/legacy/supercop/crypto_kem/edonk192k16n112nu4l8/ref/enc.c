...
int crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk)
{
    ...
    unsigned char hash[crypto_hash_sha384_BYTES], temphash[2*crypto_hash_sha384_BYTES];
    unsigned char SHA2ofC[crypto_hash_sha384_BYTES];
    unsigned char s0s1[crypto_hash_sha384_BYTES], s1s0[crypto_hash_sha384_BYTES];
    unsigned char allrandombytes[NrColumns];
    unsigned char temp01;
    
#if BinarySpan == 4
    uint8_t cosetLeader[NrRows];
...
#if BinarySpan == 4
    for(i=0; i<NrRows; i++) {
        cosetLeader[i] = PublicKey.cosetLeader[i>>1] >> 4;
        i++;
        cosetLeader[i] = PublicKey.cosetLeader[i>>1] & 0x0F;
    }
    for(i=0; i<NrRows; i++) {
        for(j=0; j<NrColumns; j++) {
            linearCombination[i][j] = PublicKey.linearCombination[i][j>>1] >> 4;
            j++;
            linearCombination[i][j] = PublicKey.linearCombination[i][j>>1] & 0x0F;
        }
    }
#endif
    ...
    randombytes(temphash, crypto_hash_sha384_BYTES);
    ...
    /* Compute s0s1 */
    crypto_hash_sha384(s0s1, temphash, crypto_hash_sha384_BYTES);
    /* Obtain s1s0 */
    ...
    for(row=0; row<NrRows; row++) {
        ...
        for(i=0; i<NrColumns; i++) {
            temp01 = cosetLeader[row] ^ linearCombination[row][i];
            for(j=0; j<BinarySpan; j++) {
                if((temp01>>j) & 1)
                    Kadd(Ciphertext.cph[i], Ciphertext.cph[i], tempproducts01[j]);
            }
            temp01 = linearCombination[row][i];
            for(j=0; j<BinarySpan; j++) {
                if((temp01>>j) & 1)
    ...
    memcpy(ct, &Ciphertext, sizeof(struct CiphertextStruct) - crypto_hash_sha384_BYTES);
    crypto_hash_sha384(SHA2ofC, ct, sizeof(struct CiphertextStruct) - crypto_hash_sha384_BYTES);

    memcpy(temphash, &s1s0, crypto_hash_sha384_BYTES);
    memcpy(&temphash[crypto_hash_sha384_BYTES], &SHA2ofC, crypto_hash_sha384_BYTES);
    crypto_hash_sha384(hash, temphash, 2*crypto_hash_sha384_BYTES);

    memcpy(Ciphertext.HashedSecretShare, &hash, crypto_hash_sha384_BYTES);
    ...
    for(i=0; i<crypto_hash_sha384_BYTES; i++) printf("%02x", Ciphertext.HashedSecretShare[i]); 
    printf("\n");
#endif

    /* Computing the SharedSecret from s0s1 and SHA2ofC */
    memcpy(temphash, &s0s1, crypto_hash_sha384_BYTES);
    memcpy(&temphash[crypto_hash_sha384_BYTES], &SHA2ofC, crypto_hash_sha384_BYTES);
    crypto_hash_sha384(ss, temphash, 2*crypto_hash_sha384_BYTES);

#ifdef KAT
    printf("\n\nEncapsulating the following shared secret:\n"); 
    for(i=0; i<crypto_hash_sha384_BYTES; i++) printf("%02x", ss[i]); 
    printf("\n");
#endif
