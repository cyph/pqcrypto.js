                                ...
                                IN const seed_t *seed) 
{
        ...
        aes256_key_t key;
        bike_static_assert(sizeof(*seed) == sizeof(key.raw),
...
_INLINE_ status_t perform_aes(OUT uint8_t *ct, IN OUT aes_ctr_prf_state_t *s) {
        ...
        bike_static_assert(((sizeof(s->ctr.qw[0]) == 8) && (BIT(33) >= MAX_AES_INVOKATION)),
        ...
        aes256_enc(ct, s->ctr.bytes, &s->ks);
                     ...
                     IN const uint32_t len)
{
    ...
    GUARD(perform_aes(s->buffer.bytes, s), res, EXIT);
