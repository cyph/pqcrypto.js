...
int crypto_kem_keypair(unsigned char *pk, unsigned char *sk)
{
	...
	memcpy(sk + CRYPTO_RANDOMBYTES, pk, crypto_kem_PUBLICKEYBYTES);
...
int crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk)
{
	...
	SHAKE256(buf + D_BYTES, HASH_LENGTH * 2, ct, crypto_kem_CIPHERTEXTBYTES);
	SHAKE256(ss, crypto_kem_BYTES, buf, D_BYTES + HASH_LENGTH * 2);
...
int crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk)
{
	unsigned char buf[D_BYTES + HASH_LENGTH * 2];
	unsigned char hash[CRYPTO_RANDOMBYTES + HASH_LENGTH];
	unsigned char ct_prime[crypto_kem_CIPHERTEXTBYTES];
	...
	/* decrypt ct to get m' */
	crypto_encrypt_open(buf, &m_prime_len, ct, crypto_kem_CIPHERTEXTBYTES, sk_cpa);
