...
int crypto_aead_encrypt(unsigned char *c, unsigned long long *clen,
	...
	if (adlen) {
		...
		while (rlen >= RATE) {
			x00 ^= U32BIG(*(u32* )(ad));
			x01 ^= U64BIG(*(u64* )(ad + 4));
			x10 ^= U32BIG(*(u32* )(ad + 12));
			x11 ^= U64BIG(*(u64* )(ad + 16));
		...
		for (i = 0; i < rlen; ++i, ++ad) {
			if (i >= 16) {
				x11 ^= INS_BYTE64(*ad, i - 16);
			} else if (i >= 12) {
				x10 ^= INS_BYTE32(*ad, i - 12);
			} else if (i >= 4) {
		...
		if (rlen >= 16) {
			x11 ^= INS_BYTE64(0x80, rlen - 16);
		} else if (rlen >= 12) {
			x10 ^= INS_BYTE32(0x80, rlen - 12);
		} else if (rlen >= 4) {
	...
	if (rlen) {
		...
		for (i = 0; i < rlen; ++i, ++m, ++c) {
			if (i >= 16) {
				x11 ^= INS_BYTE64(*m, i - 16);
				*c = EXT_BYTE64(x11, i - 16);
			} else if (i >= 12) {
				x10 ^= INS_BYTE32(*m, i - 12);
				*c = EXT_BYTE32(x10, i - 12);
			} else if (i >= 4) {
...
int crypto_aead_decrypt(unsigned char *m, unsigned long long *mlen,
	...
	if (rlen) {
		...
		for (i = 0; i < rlen; ++i, ++m, ++c) {
			if (i >= 16) {
				*m = EXT_BYTE64(x11, i-16) ^ *c;
				x11 &= ~INS_BYTE64(0xff, i - 16);
				x11 |= INS_BYTE64(*c, i - 16);
			} else if (i >= 12) {
				*m = EXT_BYTE32(x10, i-12) ^ *c;
				x10 &= ~INS_BYTE32(0xff, i - 12);
				x10 |= INS_BYTE32(*c, i - 12);
			} else if (i >= 4) {
