...
const unsigned char gamma_table[128] = {
...
void wage_permutation( unsigned char *state )
{
	...
	for ( i = 0; i < WAGEROUNDS; i++ )
	{
		...
		//Left half update
		fb_end = gamma_table[state[0]]^state[6]^state[8]^state[12]^state[13]^state[19]^state[24]^state[26]^state[30]^state[31]^wgp7_table[state[36]]^RC1[i];
