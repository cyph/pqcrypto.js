			 ...
			 )
{
  ...
  if (adlen == 0) { // Empty AD string
	...
	adlen = ad_encryption(&ad,L,s,45,adlen);
      }
      else if (adlen == 16) {// Last AD block, full
	adlen = ad_encryption(&ad,L,s,44,adlen);
      }
      else { // Normal full AD block
	adlen = ad_encryption(&ad,L,s,36,adlen);
      }
    }
  ...
  if (mauthlen == 0) { // Empty MSG string
	...
	mauthlen = ad_encryption(&m,L,s,47,mauthlen);
      }
      else if (mauthlen == 16) {// Last MSG block, full
	mauthlen = ad_encryption(&m,L,s,46,mauthlen);
      }
      else { // Normal full MSG block
	mauthlen = ad_encryption(&m,L,s,38,mauthlen);
      }
    }
  ...
  *clen = mlen + 16;
  ...
  if (mlen > 0) {
    ...
    encrypt_tag(s,Lenc,34);
    while (mlen > 16) {
      mlen = msg_encryption(&m,&c,Lenc,s,34,mlen);
    }
    rho(m, c, s, mlen, 16);
    c = c + mlen;
    m = m + mlen;
...
)
{
  ...
  *mlen = clen;
  ...
  if (clen > 0) {
    ...
    encrypt_tag(s,Lenc,34);
    while (clen > 16) {
      clen = msg_decryption(&m,&c,Lenc,s,34,clen);
    }
    irho(m, c, s, clen, 16);
    c = c + clen;
    m = m + clen;
  ...
  if (mauthlen == 0) { // Empty MSG string
    ice(s,L,47);
      ...
      if (mauthlen < 16) { // Last MSG block, partial
	mauthlen = ad_encryption(&mauth,L,s,47,mauthlen);
      }
      else if (mauthlen == 16) {// Last MSG block, full
	mauthlen = ad_encryption(&mauth,L,s,46,mauthlen);
      }
      else { // Normal full MSG block
	mauthlen = ad_encryption(&mauth,L,s,38,mauthlen);
      }
    }
