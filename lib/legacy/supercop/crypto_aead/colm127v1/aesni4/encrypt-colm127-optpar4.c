/*
 * AES-COLM_127 v1 AES-NI/AVX2 implementation
		...
		const __m128i* expkey)
{
	...
	delta = gf128_mul3(LL);
	v = _mm_set_epi64x(0x807f00, *(uint64_t*)nonce);
	v = byte_swap(v);
	v ^= delta;
     ...
     )
{
	...
	__m128i W, Wtmp, Wtag;
	__m128i block, Lup, Ldown, Ltag, Lnext;
	...
	*clen = mlen + h*16 + 16;
	...
	while (remaining > 4*16) {
		...
		/* combine Ldown management with rho and IT generation */
		Ldown1 = gf128_mul2(Lnext);
		Ldown2 = gf128_mul2(Ldown1);
		Ldown3 = gf128_mul2(Ldown2);
		Ldown4 = gf128_mul2(Ldown3);
		Lnext = Ldown4;

		// nblocks == 126: after block1
		// nblocks == 125: after block2
		// nblocks == 124: after block3
		// nblocks == 123: after block4
		// nblocks < 123: not yet
		rho(block1, W, Wtmp); nblocks++; 
		if (nblocks == 127) { itag=1; Wtag = W; Ltag = Ldown2; Ldown2 = Ldown3; Ldown3 = Ldown4; Ldown4 = Lnext = gf128_mul2(Ldown4); nblocks = 0; }
		rho(block2, W, Wtmp); nblocks++;
		if (nblocks == 127) { itag=1; Wtag = W; Ltag = Ldown3; Ldown3 = Ldown4; Ldown4 = Lnext = gf128_mul2(Ldown4); nblocks = 0; }
		rho(block3, W, Wtmp); nblocks++;
		if (nblocks == 127) { itag=1; Wtag = W; Ltag = Ldown4; Ldown4 = Lnext = gf128_mul2(Ldown4); nblocks = 0; }
		rho(block4, W, Wtmp); nblocks++;
		if (nblocks == 127) { itag=1; Wtag = W; Ltag = Lnext = gf128_mul2(Ldown4); nblocks = 0; } 
		...
		if (itag) {
			block = AES_encrypt(Wtag, expkey) ^ Ltag;
	...
	while (remaining > 16) {
		...
		if (nblocks == 127) { 
			Ldown = gf128_mul2(Ldown); 
			block = AES_encrypt(W, expkey) ^ Ldown;
			_mm_storeu_si128((__m128i*)out, byte_swap(block));
			out += 16;
			nblocks = 0;
		}
   ...
   )
{
	...
	*mlen = 0; /* is updated as we go */
	...
	while (remaining > 4*16) {
		...
		/* avoid only doing more because of IT */
		if (remaining < 2*4*16 && nblocks > 123) {
			break;
		}
		...
		block1 = byte_swap(_mm_loadu_si128((__m128i*)in)); nblocks++;
		if (nblocks == 127) { itag=1; memcpy(ibuf, in, 16); in += 16; Ltag = Ldown2; Ldown2 = Ldown3; Ldown3 = Ldown4; Ldown4 = Lnext = gf128_mul2(Ldown4); nblocks = 0; }
		block2 = byte_swap(_mm_loadu_si128((__m128i*)in+1)); nblocks++;
		if (nblocks == 127) { itag=2; memcpy(ibuf, in, 16); in += 16; Ltag = Ldown3; Ldown3 = Ldown4; Ldown4 = Lnext = gf128_mul2(Ldown4); nblocks = 0; }
		block3 = byte_swap(_mm_loadu_si128((__m128i*)in+2)); nblocks++;
		if (nblocks == 127) { itag=3; memcpy(ibuf, in, 16); in += 16; Ltag = Ldown4; Ldown4 = Lnext = gf128_mul2(Ldown4); nblocks = 0; }
		block4 = byte_swap(_mm_loadu_si128((__m128i*)in+3)); nblocks++;
		if (nblocks == 127) { itag=4; memcpy(ibuf, in, 16); in += 16; Ltag = Lnext = gf128_mul2(Ldown4); nblocks = 0; }
		...
		rho_inv(block1, W, Wtmp); if (itag == 1) { Wtag = W; }
		rho_inv(block2, W, Wtmp); if (itag == 2) { Wtag = W; }
		rho_inv(block3, W, Wtmp); if (itag == 3) { Wtag = W; }
		rho_inv(block4, W, Wtmp); if (itag == 4) { Wtag = W; }
		...
		if (itag) { 
			block = AES_encrypt(Wtag, expkey) ^ Ltag;
			_mm_storeu_si128((__m128i*)buf, byte_swap(block));
			if (memcmp(buf, ibuf, 16) != 0) { return -1; }
			remaining -= 16;
			nblocks = 0;
		}
	...
	while (remaining > 16) {
		...
		*mlen += 16;
		...
		if (nblocks == 127) { 
			...
			block = AES_encrypt(W, expkey) ^ Ldown;
			_mm_storeu_si128((__m128i*)buf, byte_swap(block));
			if (memcmp(buf, in, 16) != 0) { return -1; }
			in += 16;
			remaining -= 16;
			nblocks = 0;
		}
	...
	*mlen += remaining;
	...
	if (nblocks == 127) { 
		Ldown = gf128_mul2(Ldown); 
		Wtag = AES_encrypt(W, expkey) ^ Ldown;
		_mm_storeu_si128((__m128i*)buf, byte_swap(Wtag));
		if (memcmp(buf, in, 16) != 0) { return -1; }
		in += 16;
		nblocks = 0;
	}
