    ...
    uint8_t *tk1, const uint8_t *npub, const uint8_t *k)
{
    ...
    while (adlen > 2*BLOCKBYTES) {          // Process double blocks but the last
        ...
        skinny128_384_plus(state, state, tk1, ad+BLOCKBYTES, k);
        UPDATE_CTR(tk1);
        ad += 2*BLOCKBYTES;
        adlen -= 2*BLOCKBYTES;
    }
    ...
    if (adlen == 2*BLOCKBYTES) {            // Left-over complete double block
        ...
        skinny128_384_plus(state, state, tk1, pad, k);
        UPDATE_CTR(tk1);
    } else {
        SET_DOMAIN(tk1, 0x2C);
        UPDATE_CTR(tk1);
        ...
        if (mlen >= BLOCKBYTES) {
            skinny128_384_plus(state, state, tk1, m, k);
            if (mlen > BLOCKBYTES)
                UPDATE_CTR(tk1);
            mlen -= BLOCKBYTES;
            m += BLOCKBYTES;
    ...
    while (mlen > 32) {
        ...
        skinny128_384_plus(state, state, tk1, m+BLOCKBYTES, k);
        UPDATE_CTR(tk1);
        m += 2 * BLOCKBYTES;
        mlen -= 2 * BLOCKBYTES;
    }
    ...
    UPDATE_CTR(tk1);
    skinny128_384_plus(state, state, tk1, npub, k);
}
