...
crypto_sign_keypair(unsigned char *pk, unsigned char *sk)
{
	...
	falcon1024tree_fpu_keygen(
		&rng, f, g, F, G, h, 10, tmp.b);

	/*
	 * Expand private key.
	 */
	falcon1024tree_fpu_expand_privkey(
	 ...
	 * keys during keygen).
	 */
	falcon1024tree_fpu_to_ntt_monty(h, 10);

	/*
	 * Encode public key.
	 */
	pk[0] = 0x80 + 10;
	v = falcon1024tree_fpu_modq_encode(
	...
	const unsigned char *sk)
{
	 ...
	 */
	esig[0] = 0x20 + 10;
	sig_len = falcon1024tree_fpu_comp_encode(
	...
	const unsigned char *pk)
{
	...
	if (falcon1024tree_fpu_modq_decode(
	...
	if (falcon1024tree_fpu_comp_decode(
	 ...
	 * Verify signature.
	 */
	if (!falcon1024tree_fpu_verify_raw(
		hm, sig, h, 10, tmp.b))
	{
		return -1;
	}
