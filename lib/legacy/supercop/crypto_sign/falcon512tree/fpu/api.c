...
crypto_sign_keypair(unsigned char *pk, unsigned char *sk)
{
	...
	falcon512tree_fpu_keygen(
		&rng, f, g, F, G, h, 9, tmp.b);

	/*
	 * Expand private key.
	 */
	falcon512tree_fpu_expand_privkey(
	 ...
	 * keys during keygen).
	 */
	falcon512tree_fpu_to_ntt_monty(h, 9);

	/*
	 * Encode public key.
	 */
	pk[0] = 0x80 + 9;
	v = falcon512tree_fpu_modq_encode(
	...
	const unsigned char *sk)
{
	 ...
	 */
	esig[0] = 0x20 + 9;
	sig_len = falcon512tree_fpu_comp_encode(
	...
	const unsigned char *pk)
{
	...
	if (falcon512tree_fpu_modq_decode(
	...
	if (falcon512tree_fpu_comp_decode(
	 ...
	 * Verify signature.
	 */
	if (!falcon512tree_fpu_verify_raw(
		hm, sig, h, 9, tmp.b))
	{
		return -1;
	}
