...
crypto_sign_keypair(unsigned char *pk, unsigned char *sk)
{
	 ...
	 * Expand private key.
	 */
	falcon512tree_avx2_expand_privkey(
	 ...
	 * keys during keygen).
	 */
	falcon512tree_avx2_to_ntt_monty(h, 9);

	/*
	 * Encode public key.
	 */
	pk[0] = 0x80 + 9;
	v = falcon512tree_avx2_modq_encode(
	...
	const unsigned char *sk)
{
	 ...
	 */
	esig[0] = 0x20 + 9;
	sig_len = falcon512tree_avx2_comp_encode(
	...
	const unsigned char *pk)
{
	...
	if (falcon512tree_avx2_modq_decode(
	...
	if (falcon512tree_avx2_comp_decode(
	 ...
	 * Verify signature.
	 */
	if (!falcon512tree_avx2_verify_raw(
		hm, sig, h, 9, tmp.b))
	{
		return -1;
	}
