...
/* This file is a modified copy of evalMQSnocst_gf2.c, used for the hybrid 
   evaluation of the public key. The main modification is the macro 
   MACRO_INIT_CONSTANT, which is added in each function */
...
#define NB_EQq_orig (NB_EQ_orig/NB_BITS_UINT)
#define NB_EQr_orig (NB_EQ_orig%NB_BITS_UINT)


#if NB_EQr_orig
    #define MACRO_INIT_CONSTANT \
        /* MODIFICATION: This row is added */\
        c[NB_WORD_EQ]=pk[NB_WORD_EQ];\
        /* MODIFICATION: This row is modified */\
        pk+=NB_WORD_EQ+1;
#else
    #define MACRO_INIT_CONSTANT pk+=NB_WORD_EQ;
#endif
...
/* MODIFICATION: This row is modified */
#if NB_EQq_orig
    #define NB_EQ (NB_EQq_orig*NB_BITS_UINT)
#else
...
void PREFIX_NAME(evalMQSnocst_unrolled_quo_gf2)(vecm_gf2 c, cst_vecnv_gf2 m, cst_mqsnv_gf2m pk)
{
    UINT xi,xj;
