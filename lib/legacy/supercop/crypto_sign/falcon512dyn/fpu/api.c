...
crypto_sign_keypair(unsigned char *pk, unsigned char *sk)
{
	...
	sk[0] = 0x50 + 9;
	u = 1;
	v = falcon512dyn_fpu_trim_i8_encode(
		sk + u, CRYPTO_SECRETKEYBYTES - u, f, 9,
		falcon512dyn_fpu_max_fg_bits[9]);
	if (v == 0) {
		return -1;
	}
	u += v;
	 ...
	 */
	pk[0] = 0x00 + 9;
	v = falcon512dyn_fpu_modq_encode(
	...
	const unsigned char *sk)
{
	...
	u = 1;
	v = falcon512dyn_fpu_trim_i8_decode(
		f, 9,
		falcon512dyn_fpu_max_fg_bits[9],
	...
	if (!falcon512dyn_fpu_complete_private(
		G, f, g, F, 9, tmp.b))
	{
		return -1;
	}
	 ...
	 */
	esig[0] = 0x20 + 9;
	sig_len = falcon512dyn_fpu_comp_encode(
	...
	const unsigned char *pk)
{
	...
	if (falcon512dyn_fpu_modq_decode(
		h, 9, pk + 1, CRYPTO_PUBLICKEYBYTES - 1)
		!= CRYPTO_PUBLICKEYBYTES - 1)
	{
		return -1;
	}
	falcon512dyn_fpu_to_ntt_monty(h, 9);
	...
	if (falcon512dyn_fpu_comp_decode(
	 ...
	 * Verify signature.
	 */
	if (!falcon512dyn_fpu_verify_raw(
		hm, sig, h, 9, tmp.b))
	{
		return -1;
	}
